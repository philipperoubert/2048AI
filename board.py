...
class Board():
    ...
    def move_down(self):
        """
        Makes the move "down" on the board
        """
    ...
    def move_up(self):
        """
        Makes the move "up" on the board
        """
        self.board = self.board.transpose()
        for i in range(4):
            for j in range(1, 4):
    ...
    def move_right(self):
        """
        Makes the move "right" on the board
        """
        for i in range(4):
            for j in range(1, 5):
                if self.board[i][-j] != 0:
    ...
    def move_left(self):
        """
        Makes the move "left" on the board
        """
        for i in range(4):
            for j in range(1, 4):
                if self.board[i][j] != 0:
    ...
    def free_cells(self):
        """
        Returns the number of free cells, to be used for the search algorithm.
        """
        free_cells = np.where(self.board.reshape(-1) == 0)[0]  # list containing indexes of 0's
        return free_cells
