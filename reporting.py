...
def calculate_moves_total(moves):
    ...
    """
    total = 0
    for _, value in moves.items():
        total += value
    return total

def calculate_move_direction_percentage(moves, total):
    """
    Calculates number of moves and converts in %
    """
    percentage = []
...
def plot_game_reports(data, save_csv=True, add_csv_suffix=True, csv_filename='./data/game_report{}.csv'):
    ...
    best_moves, best_score, best_time, _, did_win, highest_tile, best_board = df.iloc[df['Score'].idxmax()]
    ...
    print('==========================================')
    worst_moves, worst_score, worst_time, _, did_win, worst_highest_tile, worst_board = df.iloc[df['Score'].idxmin()]
    ...
    print('Percentage wins: {}'.format(str(round(did_win / (loses + wins) * 100, 2)) + '%'))
...
def get_scores(scores):
    """
    Gets the scores from the runs, returns best and avg scores
    """
